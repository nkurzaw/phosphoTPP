...
Then, we retrieve our secound level of normalization factors which perform the normalization described by @savitski_2014 (originally done on protein level) which normalize peptide fold changes in the different temperature channels towards the melting curve of median values retrieved from high-quality melting peptides.
```{r}
norm_in_df <- 
...
We then filter for hiqh quality fit according to the criteria defined by @savitski_2014 and inspect reproduciability by visualization of melting point scatter plots between the different replicates
...
And again we inspect reproduciability by visualizing scatter plots of meling points estimated in the different replicates
