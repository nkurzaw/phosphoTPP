...
```{r unmodPhosphoScatter, cache=TRUE, dev=c('pdf'), warning=FALSE, message=FALSE, fig.cap="__Scatterplot of unmodified protein T$_{m}$ estimates vs. phosphopeptide-enriched sample T$_{m}$ estimates by Huang et al.__", fig.asp=1}
...
```{r unmodReps, echo=FALSE, cache=TRUE, dev=c('pdf'), warning=FALSE, message=FALSE, fig.cap="__Reproducibility of unmodified sample T$_{m}$ estimates by Huang et al.__ Displayed T$_{m}$ are represented in degree Celsius, the gray line is the identity line, plots with lightblue colored background represent technical replicates according to the labelling by Huang et al. Of note, the data could not be filtered according to the high quality criteria used by Huang et al. because this information was not annotated the supplementary table they supplied.", fig.asp=1}
huang_unmod_mat_numeric <- data.frame(
  apply(huang_bulk_tab[,2:13], 
        2, as.numeric))
...
```{r phoshoReps, cache=TRUE, dev=c('pdf'), warning=FALSE, message=FALSE, fig.cap="__Reproducibility of high quality phosphopeptide-enriched sample T$_{m}$ estimates by Huang et al.__ Displayed T$_{m}$ are represented in degree Celsius, the gray line is the identity line, plots with lightblue colored background represent technical replicates according to the labelling by Huang et al.", fig.asp=1}
...
![__Schematic illustration of the workflow as performed in this study.__]("workflow_savitski_smaller")

```{r savitskiUnModReps, cache=TRUE, dev=c('pdf'), warning=FALSE, message=FALSE, fig.cap="__Reproducibility of high quality unmodified sample T$_{m}$ estimates by the Savitski lab.__", fig.asp=1}
...
```{r savitskiPhosphoReps, echo=FALSE, cache=TRUE, dev=c('pdf'), warning=FALSE, message=FALSE, fig.cap="__Reproducibility of high quality phosphopeptide-enriched sample T$_{m}$ estimates in this study.__", fig.asp=1}
...
```{r deltaTmDist, echo=FALSE, cache=TRUE, dev=c('pdf'), warning=FALSE, message=FALSE, fig.cap="__Comparison of $\\Delta$T$_{m}$ distribution between the two studies.__"}
...
```{r pifComparison, echo=FALSE, cache=TRUE, dev=c('pdf'), warning=FALSE, message=FALSE, fig.cap="__Comparison of __"}
mxq_df_huang <- read_xlsx("../dat/Hotspot_phosphopeptides_MQ.xlsx", sheet = "evidence")
mxq_df_potel <- bind_rows(lapply(phosho_mxq_peptide_files, function(x) read_delim(x, delim = "\t")))

mxq_combo_df <- bind_rows(
  mxq_df_huang %>% dplyr::select(PIF) %>% mutate(dataset = "Huang et al.", PIF = as.numeric(PIF)),
  mxq_df_potel %>% dplyr::select(PIF) %>% mutate(dataset = "This study", PIF = as.numeric(PIF))
)

```

```{r pifComparison2, eval = F, echo=FALSE, cache=TRUE, dev=c('pdf'), warning=FALSE, message=FALSE, fig.cap="__Comparison of __"}
# compare their unmodified vs. phospho PIF
mxq_unmod_df_huang <- 
  read_xlsx("../dat/Hotspot_Bulk_phosphopeptides.xlsx", sheet = "evidence")

mxq_combo_df <- bind_rows(
  mxq_unmod_df_huang %>% dplyr::select(PIF) %>% mutate(dataset = "Huang et al. bulk phosphopeptides", PIF = as.numeric(PIF)),
  mxq_df_huang %>% dplyr::select(PIF) %>% mutate(dataset = "Huang et al. enriched phosphopeptides", PIF = as.numeric(PIF)),
  mxq_df_potel %>% dplyr::select(PIF) %>% mutate(dataset = "This study enriched phosphopeptides", PIF = as.numeric(PIF))
)

ggplot(mxq_combo_df, aes(PIF, fill = dataset)) +
  geom_density(alpha = 0.25) +
  theme_bw() +
  theme(legend.position = "bottom")
```

```{r s2iComparison3, echo=FALSE, cache=TRUE, dev=c('pdf'), warning=FALSE, message=FALSE, fig.cap="__Comparison of __"}
mxq_unmod_df_huang <- 
...
```{r compareTmOtherDat, echo=FALSE, cache=TRUE, dev=c('pdf'), warning=FALSE, message=FALSE, fig.cap="__Comparison of __"}
nbf_mean_tm_df <- nbf_tm_rep_df %>% 
  ungroup %>% 
  rowwise() %>% 
...
huang_unmod_mean_tm_df <- huang_unmod_mat_numeric %>% 
  tbl_df() %>% 
  mutate(uniprot = huang_unmod_data$`Uniprot Accession`) %>% 
  left_join(huang_mapping %>% dplyr::select(uniprot = From, gene_name = To), 
            by = "uniprot") %>% 
  rowwise() %>% 
  mutate(huang_mean_tm = mean(
    c(`Tm.Bio1_1...C.`, `Tm.Bio1_2...C.`, `Tm.Bio2_1...C.`, `Tm.Bio2_2...C.`, 
      `Tm.Bio3_1...C.`, `Tm.Bio3_2...C.`, `Tm.Bio4_1...C.`, `Tm.Bio4_2...C.`, 
      `Tm.Bio5_1...C.`, `Tm.Bio5_2...C.`, `Tm.Bio6_1...C.`, `Tm.Bio6_2...C.`),
    na.rm = TRUE)) %>% 
  ungroup() %>% 
  dplyr::select(gene_name, huang_mean_tm) %>% 
  group_by(gene_name) %>% 
